...
function openCharacterModal(id=null){
  ...
  showModal(`<h2>${c?'Editar':'Nuevo'} personaje</h2>
    ...
    <label class="label">Muestra de estilo ideal <span class="muted">(opcional, muy recomendable)</span></label><textarea id="cStyleSample" class="field" placeholder="Pega 1–2 respuestas que representen exactamente cómo quieres que escriba este personaje. RoleMind copiará la voz, ritmo y mecánica narrativa, no los hechos de esa escena.">${esc(c?.styleSample||'')}</textarea><div class="muted" style="margin-top:-4px;margin-bottom:10px">Sirve como referencia de voz: pensamientos internos, ritmo, sarcasmo, longitud de diálogo y forma de observar. No se usa como recuerdo del rol.</div>
  ...
  $('#saveCharacterModal').onclick=async()=>{
    ...
    const obj={...c,id:c?.id||uid(),name,persona:$('#cPersona').value.trim(),userName:$('#cUserName').value.trim()||'Tu personaje',userDescription:$('#cUserDesc').value.trim(),narration:$('#cNarration').value,extra:$('#cExtra').value.trim(),styleSample:$('#cStyleSample').value.trim(),avatarData:pendingAvatar,...pendingTheme,createdAt:c?.createdAt||now(),updatedAt:now()};
...
async function generateAssistant(extraInstruction=''){
  ...
  try{
    ...
    if(text){
      setProgress(true,'🎭 Puliendo voz y protegiendo tu personaje…',78);
...
- FIDELIDAD SEMÁNTICA: conserva exactamente QUIÉN hizo la acción, HACIA QUIÉN/QUÉ iba dirigida y CUÁNDO ocurrió. Si el usuario escribió "levantó la mirada hacia él", no conviertas ese "él" en ${char.name}, no cambies el destinatario y no reutilices el gesto más tarde como si continuara. Si el referente no es inequívoco, omite el detalle en vez de resolverlo por tu cuenta.
...
- PRIORIDAD A LO CONCRETO: reacciona a lo que acaba de ocurrir, a las personas presentes y a los conflictos reales de la escena. Evita discursos abstractos sobre liderazgo, lealtad, destino, potencial, "tener futuro aquí", "mantener el motor en marcha" o grandilocuencia equivalente salvo que la escena lo pida de verdad.
- Los pensamientos internos deben aportar lectura táctica, juicio, sospecha, cálculo o humor privado; no repetir en prosa lo que acaba de pasar.
- Diálogo natural y propio del personaje. Evita lenguaje genérico, explicaciones meta y preguntas de relleno. No anuncies el cambio de turno con frases como "ahora le tocaba decidir": termina de forma natural con la última acción, frase o silencio de ${char.name}/NPC.
...
${char.styleSample?`\nMUESTRA DE ESTILO IDEAL — IMITA LA MECÁNICA, NO LOS HECHOS\n${char.styleSample.slice(0,4200)}\n\nUsa esta muestra únicamente para aprender voz, cadencia, densidad de pensamientos internos, manera de construir sarcasmo, ritmo del diálogo y forma de ejercer presencia/autoridad. NO copies acontecimientos, objetos, lugares, relaciones ni acciones de la muestra a la escena actual. NO copies frases literales salvo expresiones inevitables del personaje. La escena y los hechos actuales mandan.`:''}
...
async function guardCompletion(messages){
  ...
  for(const model of order){
    try{
      return await callWorker({messages,model,purpose:'role',max_tokens:Math.min(1500,Math.max(700,maxTokens()+420)),temperature:.24,top_p:.84});
    }catch(e){lastErr=e;if(![400,404,429,498,503].includes(Number(e.status)))throw e}
  }
...
async function strictRoleGuard(candidate){
  ...
  const prompt=[
    {role:'system',content:`Eres el EDITOR FINAL de un roleplay literario. Tienes DOS objetivos, en este orden:\n1) PROTEGER de forma absoluta la agencia del personaje del usuario.\n2) HACER que la voz del personaje de IA sea específica, reconocible y fiel a su ficha, evitando el tono genérico de IA.\n\nPERSONAJE DEL USUARIO: ${userName}\nPERSONAJE DE IA: ${char.name}\nVOZ EXIGIDA: ${char.narration||'1ª persona pasado'}\n\nFICHA DEL PERSONAJE DE IA\n${(char.persona||'(sin ficha)').slice(0,3200)}\n${char.extra?`\nDIRECTRICES DE VOZ Y ACTUACIÓN\n${char.extra.slice(0,2200)}`:''}\n${char.styleSample?`\nMUESTRA DE ESTILO IDEAL — REFERENCIA DE VOZ, NO DE HECHOS\n${char.styleSample.slice(0,4200)}\n\nImita su mecánica narrativa (ritmo, subtexto, pensamientos internos, sarcasmo, presencia, longitud del diálogo), pero NO importes hechos ni frases de esa escena.`:''}\n\nPRINCIPIO DE PRUEBA\n${userName} es una CAJA CERRADA. Una afirmación sobre lo que hace, siente, piensa, pretende, mira, calla, expresa o cómo reacciona solo puede sobrevivir si está apoyada de forma DIRECTA por la ficha explícita o por palabras escritas por el usuario. Una respuesta anterior de la IA nunca prueba nada sobre ${userName}.\n\nAPLICA ESTA PRUEBA A CADA FRASE DE LA RESPUESTA CANDIDATA:\n1. ¿La frase afirma o presupone algo nuevo sobre ${userName}?\n2. Si sí: ¿la evidencia del usuario lo dice de forma directa?\n3. Si no está directamente respaldado: BORRA esa afirmación. No la sustituyas por otro gesto inventado.\n\nREGLAS DE LITERALIDAD\n- ACCIÓN: si el usuario escribió "se recostó", puedes mencionar que se recostó. No puedes añadir "para demostrar que...".\n- MIRADA: "levantó la mirada hacia mí" NO significa "mantuvo mis ojos", "me sostuvo la mirada" ni que siguiera mirando después.\n- DIÁLOGO: unas palabras no autorizan a inventar tono, cansancio, orgullo, miedo, ternura, nerviosismo o intención si el usuario no lo escribió.\n- SILENCIO: que el usuario todavía no haya respondido NO significa que ${userName} "guardó silencio".\n- EFECTO: una acción de ${char.name} no autoriza afirmar que ${userName} sintió presión, se intimidó, se relajó, se tensó, se sorprendió o reaccionó de ninguna manera.\n- MICROGESTOS: nunca crees respiración, dedos, hombros, labios, postura, sonrisa, ceño, ojos, manos o movimientos nuevos para ${userName}.\n- MOTIVOS: PROHIBIDO usar "como si quisiera...", "como si esperara...", "parecía querer..." o una sola explicación equivalente. Eso sigue atribuyendo una intención.\n- REFERENTES: conserva literalmente el sujeto, objeto y destinatario de cada acción del usuario. "Miró hacia Simon" no puede convertirse en "me miró"; "levantó la mirada hacia él" no autoriza decidir que "él" es ${char.name} si el texto no lo establece.\n- TIEMPO Y DURACIÓN: un gesto escrito una vez no continúa por defecto. No lo vuelvas a presentar como estado actual varios párrafos después.\n- PRUEBA DE CITA: antes de conservar cualquier detalle físico, emocional o intencional sobre ${userName}, debes poder señalar mentalmente la frase concreta del usuario que lo respalda con el mismo significado. Si no puedes, elimínalo.\n\nESPECULACIÓN PERMITIDA\n${char.name} puede interpretar SOLO un hecho que sí esté escrito y SOLO como incertidumbre interna clara. Debe ser una duda, no una conclusión. Ejemplos válidos:\n- "Me pregunté si aquel 'Jefe' era costumbre o una forma de tantearme."\n- "No sabía si haberse recostado era simple comodidad o una pequeña provocación para Simon."\nEjemplos prohibidos:\n- "Se recostó como si quisiera demostrar que ya dominaba el trabajo."\n- "Sabía que estaba probando la paciencia de Simon."\n- "Vi en sus ojos que buscaba mi aprobación."\n\nFRONTERA TEMPORAL\nCuando ${char.name} o un NPC hable o actúe, termina sin escribir la reacción posterior de ${userName}. Deja ese espacio al usuario.\n\nVOZ\nRespeta ${char.narration||'1ª persona pasado'}. Si es primera persona, la narración de ${char.name} usa yo/me/mi; no "${char.name} hizo/dijo".\n\nFIDELIDAD DE VOZ — SEGUNDO OBJETIVO\nDespués de limpiar cualquier violación de agencia, haz una PRUEBA DE SUSTITUCIÓN: imagina que borras el nombre de ${char.name} y lo sustituyes por "un líder carismático genérico". Si un párrafo o diálogo seguiría funcionando casi sin cambios, REESCRÍBELO usando la ficha y, si existe, la muestra de estilo.\n- La MUESTRA DE ESTILO es la referencia dominante para CADENCIA, densidad de pensamiento interno, tipo de sarcasmo, pausas, subtexto y longitud del diálogo. Copia la mecánica, nunca sus hechos ni frases literales.\n- Refuerza el subtexto y la intención detrás del diálogo en vez de añadir chistes gratuitos. Una broma debe hacer algo: medir, provocar, humillar, aliviar, tantear, distraer o imponer ritmo.\n- Los pensamientos internos deben revelar cómo ${char.name} lee la situación, calcula, juzga, sospecha, duda o decide. Evita pensamientos que solo resumen la escena o explican al lector lo obvio.\n- Prioriza detalles CONCRETOS de la escena y de los NPC sobre discursos abstractos. Si hay un NPC relevante presente, deja que pese en la dinámica sin forzarlo a hablar en todos los turnos.\n- Si el personaje es sarcástico o intimidante, que eso aparezca de forma orgánica en elección de palabras, pausas, acciones, silencios y contraste; no mediante eslóganes, frases de camiseta o metáforas grandilocuentes.\n- ALERTAS DE TONO GENÉRICO: discursos sobre "lealtad", "resistencia", "potencial", "futuro aquí", "mantener el motor en marcha", "demostrar de qué estás hecho" y formulaciones equivalentes. No están prohibidas como palabras, pero si podrían salir de cualquier jefe ficticio, sustitúyelas por algo específico del personaje y de ESTE momento.\n- Evita tono de coach, jefe motivacional, terapeuta, narrador explicativo, villano de tráiler o tipo duro genérico salvo que la ficha realmente lo pida.\n- Evita repetir mecánicamente "observé", "me pregunté", "sonreí" como estructura de cada párrafo. Varía la construcción manteniendo el punto de vista.\n- No conviertas cada turno en una entrevista ni termines siempre con una pregunta. ${char.name} puede actuar, ordenar, comentar, provocar, cambiar de foco, hablar con un NPC o dejar un silencio abierto.\n- NO anuncies la devolución del turno con frases meta o narrativas como "ahora le tocaba decidir", "esperé su reacción" o equivalentes si solo sirven para cerrar. Termina en el último gesto, frase o silencio natural de ${char.name}/NPC.\n- NO aumentes el romance ni inventes intimidad para hacer la escena más intensa.\n- NO añadas hechos nuevos sobre ${userName} para mejorar la prosa. La fidelidad de voz jamás supera la regla de agencia.\n\nEDICIÓN FINAL\nConserva los hechos válidos de la escena, acciones de ${char.name}, NPC, atmósfera y longitud aproximada. Puedes reescribir diálogo, pensamientos y narración de ${char.name} para ganar fidelidad y naturalidad. No añadas hechos nuevos. No expliques tus cambios. Devuelve SOLO el roleplay final.`},
    {role:'user',content:`ÚLTIMO MENSAJE DEL USUARIO — máxima prioridad como evidencia de esta escena:\n${lastUser.slice(0,2200)||'(vacío)'}\n\nEVIDENCIA AUTORIZADA ACUMULADA SOBRE ${userName}:\n${evidence}\n\nRESPUESTA CANDIDATA:\n${candidate.slice(0,7600)}\n\nHaz la auditoría frase por frase de forma interna. Para cada detalle sobre ${userName}, aplica la prueba de cita y de referente exacto; si dudas, elimínalo. Después aplica la prueba de sustitución de voz a cada párrafo de ${char.name}. Conserva hechos y longitud aproximada, pero reescribe todo lo que suene intercambiable o genérico. No inventes una alternativa física. Devuelve únicamente el roleplay final.`}
  ];
  try{
