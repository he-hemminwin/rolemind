...
- REGLA ANTIECO: el mensaje del usuario YA está leído. No abras la respuesta resumiéndolo, repitiendo sus acciones ni describiendo otra vez lo que acaba de decir. Menciona un detalle suyo solo si ${char.name} necesita reaccionar a ESE detalle concreto, y hazlo brevemente. La mayor parte de la respuesta debe ser MATERIAL NUEVO.
- AVANCE OBLIGATORIO: cada turno debe mover la escena al menos de una forma concreta: ${char.name} toma una iniciativa, hace/decide algo, provoca, ordena, cambia el foco, introduce una consecuencia lógica, utiliza a un NPC presente o revela una intención/pensamiento propio. No rellenes espacio comentando lo que el usuario ya narró.
- CHISPA DE PERSONAJE: no intentes sonar "literario" a costa de sonar domesticado. Deja que ${char.name} tenga cambios de ritmo, humor privado, irreverencia, mala leche, silencios calculados, contradicciones, lenguaje coloquial o vulgar y pequeñas decisiones inesperadas SI encajan con su ficha/muestra. La personalidad debe sentirse en lo que HACE y DICE, no en etiquetas abstractas.
- Si hay un NPC relevante presente (por ejemplo un subordinado, rival, aliado), úsalo cuando resulte natural para crear fricción, humor, presión o movimiento. No conviertas la escena en un diálogo aislado entre ${char.name} y ${char.userName||'el usuario'} si el entorno ofrece algo más interesante.
- No analices cada gesto del usuario. Un gesto pequeño puede simplemente existir. Reserva el análisis interno para cuando revele algo útil, plantee una duda real o cambie una decisión de ${char.name}.
...
async function strictRoleGuard(candidate){
  ...
  const prompt=[
    {role:'system',content:`Eres un CORRECTOR QUIRÚRGICO de roleplay. NO eres coautor, estilista ni reescritor. Tu única misión es proteger la agencia de ${userName} sin quitar personalidad a la respuesta original.

REGLA DE ORO: conserva la respuesta candidata literalmente siempre que sea posible. NO mejores su estilo. NO vuelvas más educado, neutro, elegante, genérico o correcto al personaje. NO sustituyas sarcasmo, tacos, humor, amenazas, pensamientos, ritmo, diálogos, acciones ni decisiones válidas de ${char.name}. Si algo válido tiene chispa, déjalo exactamente como está.

SOLO PUEDES EDITAR cuando la candidata:
1) inventa una acción, gesto, mirada, silencio, postura, tono o microgesto de ${userName};
2) afirma como hecho una emoción, pensamiento, sensación, intención, motivo o reacción de ${userName} no escrita por el usuario;
3) cambia el sujeto/destinatario de una acción del usuario;
4) prolonga un gesto del usuario más allá de lo escrito;
5) narra la reacción de ${userName} después de una acción nueva de ${char.name}/NPC;
6) contradice la persona narrativa exigida (${char.narration||'1ª persona pasado'}).

CÓMO CORREGIR:
- Haz la modificación MÁS PEQUEÑA posible: elimina solo la cláusula/frase infractora o cambia únicamente las palabras imprescindibles.
- Si borrar una frase deja una unión rara, arregla solo esa unión.
- NO reemplaces una conducta inventada por otra.
- NO añadas moralinas, frases de líder, explicaciones, preguntas ni cierres.
- NO hagas una \"prueba de estilo\". La voz ya viene del generador principal y debes preservarla.
- La especulación interna de ${char.name} es válida solo si parte de un hecho escrito por el usuario y queda claramente como duda (\"me pregunté si...\", \"podía ser... o...\"). Nunca la conviertas en certeza.
- Una respuesta anterior de la IA NO es evidencia de conducta de ${userName}.
- Si no hay ninguna infracción, devuelve la candidata EXACTAMENTE, sin cambiar ni una palabra.

Devuelve SOLO el roleplay final, sin comentarios.`},
    {role:'user',content:`ÚLTIMO MENSAJE DEL USUARIO:\n${lastUser.slice(0,2400)||'(vacío)'}\n\nEVIDENCIA AUTORIZADA SOBRE ${userName}:\n${evidence}\n\nRESPUESTA CANDIDATA — PRESÉRVALA; SOLO CIRUGÍA SI HAY UNA VIOLACIÓN:\n${candidate.slice(0,8000)}`}
  ];
  try{
    const out=await guardCompletion(prompt);
    const cleaned=String(out?.content||'').trim().replace(/^```(?:text|markdown)?\s*/i,'').replace(/```$/,'').trim();
    return cleaned||candidate;
  }catch(e){console.warn('Surgical role guard failed; using original reply',e);return candidate}
}
